...
def run():
    ...
    with c2:
        ...
        with st.expander("See Distribution of Delta T"):
            ...
            hist, edges = np.histogram(dt_irreg, density = True, range = (np.min(dt_irreg), np.max(dt_irreg)))
            p_dt_irreg = figure(
                max_height=300,
                height_policy='max')
            p_dt_irreg.quad(top=hist, bottom=0, left=edges[:-1], right=edges[1:], line_color="white")
    ...
    with c1:
        ...
        with st.expander("See Distribution of Delta T"):
            ...
            hist, edges = np.histogram(dt, density = True, range = (np.min(dt_irreg), np.max(dt_irreg)))
            p_dt_reg = figure(
                max_height=300,
                height_policy='max')
            p_dt_reg.quad(top=hist, bottom=0, left=edges[:-1], right=edges[1:], line_color="white")
